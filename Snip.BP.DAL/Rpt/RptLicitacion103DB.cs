...
{
    public class RptLicitacion103DB
    {
        public static DataSet GetList(int anio, int codInstitucion)
        {
            DataSet ds = new dsRptLicitaciones103();

            using (SqlConnection conexion = new SqlConnection(AppConfiguration.ConnectionString))
            {
                using (SqlCommand command = new SqlCommand("[rpt].BpsListObrasPIPValidacionRegistro", conexion))
                {
