...
{
    public class RptLicitacion105DB
    {
        public static DataSet GetList(int anio, int codInstitucion)
        {
            DataSet ds = new dsRptLicitaciones105();

            using (SqlConnection conexion = new SqlConnection(AppConfiguration.ConnectionString))
            {
                using (SqlCommand command = new SqlCommand("[rpt].BpsListLicitacionesAContratarPorTrimestre2", conexion))
                {
